...
class _TourDetailPage extends State<TourDetailPage> {
                ...
                onPressed: (){
                  showDialog(
                    ...
                    builder: (context) {
                      return AlertDialog(
                        title: Text('후기 쓰기'),
                        content: TextField(controller: _reviewTextController,),
                        actions: <Widget>[
                          TextButton(
                            onPressed: (){
  ...
  getGoogleMap() {
    return SizedBox(
      ...
      child: GoogleMap(
        mapType: MapType.normal,                          // 지도의 형태
        initialCameraPosition: _googleMapCamera!,         // 지도에 표시할 위치
        onMapCreated: (GoogleMapController controller) {  // 지도를 표시한 다음 호출되는 콜백 함수
          _controller.complete(controller);
        },
        markers: Set<Marker>.of(markers.values),          // 지도에 표시할 마커
      ),
    );
  }
